...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (user) {
      login({ email });
      if (rememberMe) {
        localStorage.setItem("checkmate-remember", JSON.stringify({ email, password }));
      } else {
        localStorage.removeItem("checkmate-remember");
      }
      router.push("/todo");
    } else {
      ...
      <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl p-10 w-full max-w-md border border-gray-200 animate-pop-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              type="email"
              className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none bg-white text-black"
              value={email}
